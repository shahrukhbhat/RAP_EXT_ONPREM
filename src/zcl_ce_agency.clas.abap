CLASS zcl_ce_agency DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES t_agency_range TYPE RANGE OF zbp_agency_eup=>tys_agency_type.
    TYPES t_business_data TYPE zbp_agency_eup=>tyt_agency_type.
    TYPES t_business_data_external TYPE TABLE OF zrap_ce_agencies.
...
CLASS ZCL_CE_AGENCY IMPLEMENTATION.
...
*                                              i_authn_mode = if_a4c_cp_service=>service_specific ) ).
    ...
    DATA(lo_destination) = cl_http_destination_provider=>create_by_comm_arrangement(
                                             comm_scenario  = 'Z_AGENCY_ODATA_CSCEN'
                                             comm_system_id = 'Z_AGENCY_ODATA_CSCEN'
                                             service_id     = 'Z_AGENCY_ODATA_REST' ).
    http_client = cl_web_http_client_manager=>create_by_http_destination( lo_destination ).

    odata_client_proxy = /iwbep/cl_cp_factory_remote=>create_v4_remote_proxy(
      EXPORTING
         is_proxy_model_key       = VALUE #( repository_id       = 'DEFAULT'
                                             proxy_model_id      = 'ZBP_AGENCY_EUP'
                                             proxy_model_version = '0001' )
        io_http_client             = http_client
        iv_relative_service_root   = '/sap/opu/odata4/sap/zsb_agency/srvd_a2x/sap/zsd_agency/0001/' ).
